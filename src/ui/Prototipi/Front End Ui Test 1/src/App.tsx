...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubAI = bridge.on('ai.response', (payload) => {
      setBotState('success');
      setTimeout(() => setBotState('idle'), 2000);
      addBotMessage(payload.content, 'bot');
    });

    const unsubAIStream = bridge.on('ai.stream', (payload) => {
      setBotState('typing');
      // Qui si potrebbe implementare lo streaming reale dei caratteri
    });
  ...
  const addBotMessage = (text: string, type: 'bot' | 'advisory') => {
    setMessages(prev => [...prev, { 
      id: Date.now(), 
      type, 
      text, 
  ...
  const handleCommand = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && inputValue.trim() && botState === 'idle') {
      ...
      // REAL BACKEND CALL (Aura Protocol)
      bridge.send('ai.prompt', { prompt: command });
    }
  };
