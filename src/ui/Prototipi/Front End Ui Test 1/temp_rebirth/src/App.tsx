...
import { BotFace, BotState } from './components/BotFace';
import { motion, AnimatePresence } from 'motion/react';

type Message = {
  id: number;
  type: 'system' | 'advisory' | 'user' | 'bot';
  text?: string;
  time?: string;
  telemetry?: boolean;
};
...
export default function App() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    ...
    { id: 4, type: 'advisory' },
    { id: 5, type: 'user', text: 'Analyze stereo field and spectral density before saturation application.', time: '14:22:15' },
    { id: 6, type: 'bot', text: 'Accessing telemetry... Stereo width is currently correlated at +0.62. Phase alignment looks optimal for broadcast delivery.', telemetry: true }
  ]);
  ...
  const simulateTyping = async (text: string, messageId: number, endState: BotState = 'idle') => {
    setBotState('typing');
    let currentText = '';
      ...
      setTimeout(() => {
        ...
        if (command.toLowerCase().includes('error') || command.toLowerCase().includes('fail')) {
          ...
          setMessages(prev => [...prev, {
            id: newMsgId + 1,
            type: 'system',
            text: `[${timeStr}] CRITICAL ERROR: COMMAND NOT RECOGNIZED IN CURRENT CONTEXT.`
          }]);
          setTimeout(() => setBotState('idle'), 2000);
          return;
        } else if (command.toLowerCase().includes('triste') || command.toLowerCase().includes('sad') || command.toLowerCase().includes('male')) {
          responseText = "Rilevo un'anomalia emotiva nei miei circuiti... La densità spettrale è cupa. Cercherò di ottimizzare il segnale per risollevare la situazione.\n\n[ HB + ]";
          endState = 'sad';
        } else if (command.toLowerCase().includes('eq') || command.toLowerCase().includes('master')) {
          responseText = "Analyzing frequency spectrum... Applying dynamic EQ curve to tame the 2-4kHz harshness. Saturation stage engaged for harmonic warmth. \n\n[ HB + ]";
        } else {
          responseText = "Command received. Adjusting internal parameters to optimize audio flow. The neural engine is adapting to your signal.\n\n[ HB + ]";
        }

        simulateTyping(responseText, newMsgId + 1, endState);
      }, 1500);
    }
  };
  ...
  const executeSuggestedChain = () => {
    ...
    setBotState('loading');
    setTimeout(() => {
      simulateTyping("Executing suggested chain. Dynamic dip of -2.4dB applied at 300Hz. Transient preservation locked at 84%. Spectral clarity increased.\n\n[ HB + ]", Date.now());
    }, 1000);
  };
    ...
    <div className="bg-background select-none h-screen w-screen overflow-hidden text-[#e5e2e1] font-['Space_Grotesk']">
      ...
      <aside className="fixed left-0 top-12 h-[calc(100vh-3rem)] w-20 flex flex-col items-center py-4 bg-[#131313] border-r border-[#222222] z-40">
        ...
        <div className="flex flex-col w-full">
          ...
          <div className="text-[#4d4d4d] py-4 flex flex-col items-center gap-1 hover:text-[#FFB000] hover:bg-[#1a1a1a] transition-all cursor-pointer">
            <span className="material-symbols-outlined text-xl">settings_input_component</span>
            <span className="font-['Space_Grotesk'] font-mono text-[10px] uppercase">COMP</span>
          </div>
          <div className="text-[#4d4d4d] py-4 flex flex-col items-center gap-1 hover:text-[#FFB000] hover:bg-[#1a1a1a] transition-all cursor-pointer">
            <span className="material-symbols-outlined text-xl">linear_scale</span>
            <span className="font-['Space_Grotesk'] font-mono text-[10px] uppercase">LIMIT</span>
          </div>
          <div className="text-[#4d4d4d] py-4 flex flex-col items-center gap-1 hover:text-[#FFB000] hover:bg-[#1a1a1a] transition-all cursor-pointer">
            <span className="material-symbols-outlined text-xl">graphic_eq</span>
            <span className="font-['Space_Grotesk'] font-mono text-[10px] uppercase">EQ</span>
          </div>
          <div className="text-[#4d4d4d] py-4 flex flex-col items-center gap-1 hover:text-[#FFB000] hover:bg-[#1a1a1a] transition-all cursor-pointer">
            <span className="material-symbols-outlined text-xl">analytics</span>
            <span className="font-['Space_Grotesk'] font-mono text-[10px] uppercase">METERS</span>
          </div>
        </div>
      </aside>
          ...
          <div className="flex-1 p-6 font-mono overflow-y-auto custom-scrollbar space-y-6">
            ...
            {messages.map((msg) => {
              ...
              if (msg.type === 'advisory') {
                return (
                  <div key={msg.id} className="animate-advisory-in group relative mt-4 mb-6">
              ...
              if (msg.type === 'bot') {
                ...
                return (
                  <div key={msg.id} className="flex gap-4 mt-6">
                    {/* The animated Face placed next to the bot response */}
          ...
          <div className="h-14 bg-[#131313] border-t border-[#222222] flex items-center px-6 gap-4">
            ...
            <div className="flex-1 relative flex items-center">
              <input 
                ...
                placeholder={botState !== 'idle' ? "PROCESSING..." : "Type command (e.g. /apply_chain, /analyze_spectral)..."} 
                type="text"
                value={inputValue}
      ...
      <footer className="fixed bottom-0 left-0 w-full z-50 flex justify-between items-center h-10 border-t border-[#222222] bg-[#0d0d0d] px-6">
        ...
        <div className="flex items-center gap-4">
          <div className="text-[10px] font-mono text-[#FFB000] animate-pulse">
            [14:24:12] PROCESSING_READY // CHAIN_OPTIMIZED
          </div>
          <div className="h-4 w-4 bg-[#FFB000] led-amber-active"></div>
        </div>
      </footer>
    </div>
  );
